...
def json_save(url,headers): # 爬取正文，生成json并保存
    ...
    title = soup.select('h2[id="APP-Title"]')
    if title:
        title = title[0].text.strip()
    print('title:',title)

    title_sub = soup.select('p[id="APP-Subtitle"]')
    if title_sub:
        title_sub = title_sub[0].text.strip()
    else:
        title_sub = ''
    
    s = soup.select('p[id="APP-Author"]')
    if s:
        s = s[0]
...
# http://www.81.cn/jfjbmap/content/2019-03/01/content_228400.htm
    ...
    s = soup.select('div[id="APP-Content"] p ')
    content = ''
    author_org = ''
    for s1 in s:
        s2 = s1.text.strip()
        if s2[0:4] == '（作者：' or s2[0:4] == '（作者系' or s2[0:5] =='（作者单位' or s2[0:6] =='（作者分别为' :
            author_org = s2[1:-1]
            author_org = re.sub('作者单位：', '', author_org)
    ...
    s = re.findall('\d\d\d\d-\d\d/\d\d/content_.*', url)[0] 
    s = re.sub('/', '-', s)
    s = re.sub('.htm', '', s)
    s = 'jfjb_' + s
